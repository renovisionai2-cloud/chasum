...
**Snapshot date:** 2026-09-26. **Updated by:** Codex for the bounded Issue #110 provider-off subscription isolation candidate, using the Product Owner / supervisor dispatch. Earlier acceptance records remain intact.
...
| Latest accepted behavior-changing application release | **PR #107 — MERGED / PRODUCTION PRICING ACCEPTED** at `1b11ade192bb2085d0325383b3f00f9fe97fda51`, tree `99786474cc43248dbf680b0ebbe6637cc70b2308`, per [PO/supervisor closeout](https://github.com/renovisionai2-cloud/chasum/pull/107#issuecomment-5849507275). Local HEAD and origin/main matched this base at entry; no fresh serving-runtime/network query in this sandbox. Earlier serving observations remain dated history. |
...
| Current program phase | **Issue #110 / Gate B P2A provider-off subscription isolation candidate ACTIVE.** Configuration/capability foundation only; NOT a working checkout/reconciler. PR #107 pricing presentation is complete and accepted; no pricing redo. |
...
| Dependency findings / pending triage | PR #105 disclosed npm's 25 findings (3 low / 8 moderate / 13 high / 1 critical) on the unchanged lockfile. Runtime reachability/exploitability **UNASSESSED**; read-only triage pending. This is not evidence of compromise or authorization to fix/upgrade dependencies. Issue #109 assessment remains open; no audit/network/upgrade work in #110. Issue #108 theme deferral remains open and untouched. |
| Exact next substantive gate | Supervisor inspects/publishes exact #110 reviewed bytes for required hosted checks; Claude independently reviews before merge. Provider activation, actual checkout and terms remain gated. No push/PR/merge in this sandbox. Actual customer checkout requires a new Competitive Product Gate; before later schema work reconcile 037/038 history/RLS/grants. Genuine bounded team access remains required wherever sold/needed; commercial launch readiness is not established. |
| Product Owner input | No new PO decision needed for this bounded foundation. First 25 means Alpha signups, not payers. Enrollment order, historic promises, internal/test counting, cancellation/rejoining/refunds/tax and lifetime continuity remain UNDECIDED; do not infer offer eligibility from `private_alpha_enabled`. Provider activation remains gated. |
...
**Issue #110 boundary:** Dedicated server-only SaaS configuration reads `CHASUM_SAAS_BILLING_MODE` (missing = disabled), `CHASUM_SAAS_STRIPE_SECRET_KEY`, `CHASUM_SAAS_STRIPE_WEBHOOK_SECRET`, and `CHASUM_SAAS_NEW_PURCHASES_ENABLED` (only explicit `true`). No legacy/public fallback. Configured Production requires live mode/live key; recognized Preview/development require test mode/test key. Test mode in Production returns `test_requires_nonproduction`; unknown identity is unavailable. Disabled mode is allowed everywhere with no capabilities; non-Vercel support is not implemented. Configuration syntax and actual capabilities are separate; actual checkout/reconciliation remain OFF. Only synthetic implemented-capability tests demonstrate checkout-off/reconciliation-on. Future subscriber rollback must use the new-purchase control while keeping valid configuration/reconciliation healthy; disabling billing mode or removing keys stops both and is not that rollback. Existing mock provider, mutation guards, commerce, pricing and schema are unchanged. Competitive gate NOT_APPLICABLE: internal configuration/isolation, no user-facing change.
