...
**2026-09-21 current control state:** Issue #72 is CLOSED / PRODUCTION ACCEPTED. Issue #73 Package A remains **COMPLETE / PRODUCTION ACCEPTED CODE FOUNDATION**. PR #82 Package B1 is **MERGED** at main `6b095cbf8c42664a44398c01b2b19ff2e1f15c40`; normal Vercel Production deployment is SUCCESS. The exact audited B1 migration SHA-256 `57b79680fdc60ad5789d90d1d9e6e9600b9b15fb84002687e86c1130acd4dc9f` is **APPLIED + VERIFIED on Staging only** (`wnfahklzaxirftyskctd`), with RLS/FORCE RLS, zero API policies, service_role SELECT-only, revoked API helper EXECUTE and zero import rows verified. Production Supabase has NOT received B1. B2 is NOT AUTHORIZED. Issue #81 multi-location architecture is PO-locked; Issue #83 Stage 1A-Operator is the next bounded engineering task. Outside Private Alpha readiness remains IN PROGRESS; Phase 5/observability COMPLETE; Sentry OFF; Gate B not begun.
...
| First bounded task | **Issue #83 Stage 1A-Operator.** Converge the internal Business Service Catalog and `service_locations` truth across Services, Reception and Booking Sheet only. Public booking and availability SQL stay unchanged until Stage 1B; B2 remains NOT AUTHORIZED. |
...
2. **Outside-tenant onboarding/import contract:** #72 is Production accepted; #73 Package A is Production accepted and B1 is merged with Staging schema acceptance complete. Production B1 migration remains unapplied; B2 remains separately governed and NOT authorized. #81/#83 multi-location semantics must be represented before B2 operational writes.
