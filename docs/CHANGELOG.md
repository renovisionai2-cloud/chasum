...
`createInvoiceForAppointment` now treats stored `appointments.price_cents` as the tax-exclusive subtotal and adds stored `tax_cents` for the invoice/line total, balance, and paid/partial/open status. Explicit stored `price_cents = 0` is preserved; catalog fallback remains only for null/absent stored price. Nonzero `discount_cents` is copied through as a field without a new discount formula. Existing invoices are not rewritten. Unmerged candidate; no hosted or historical invoice repair.
