...
**Phase 6.0B PO-accepted.** **Phase 6.1 = PO ACCEPTED.** **Phase 6.2A = PO ACCEPTED** after hands-on Preview E2E (RCT-0006). **Phase 6.2B = PO ACCEPTED** after hands-on Preview (Chase money/refund + Sum historical recovery; tip `5f055cf`). **Phase 6.3 discovery complete / implementation NOT STARTED.** No Stripe Elements. No migrations.

Sequence: 6.0 foundation → 6.0A collectibility → **6.0B calendar sync + refund UX** → **6.1 front-desk payments** → **6.2A invoice/receipt workspace (PO accepted)** → **6.2B document integrity + lifecycle** → 6.3 outstanding / follow-up truth → 6.4 online payment completion (PO-gated).

## Next step after Chapter 6 Phase 6.2B

Phase 6.2B is **PO ACCEPTED**. Phase 6.3 **discovery** is in [`WORLD_CLASS_PHASE_6_3_DISCOVERY.md`](./WORLD_CLASS_PHASE_6_3_DISCOVERY.md). Do **not** start Phase 6.3 **implementation** until PO accepts that scope and required policy decisions. Unique `(appointment_id)` / `(transaction_id)` and atomic sequence RPC remain PO/database decisions. Historical USD cleanup, package entitlement, no-show collectibility, cancellation→invoice policy, Stripe/public online payment, and staff payment RBAC remain deferred. Do **not** start Chapter 7. Do **not** reopen accepted Chapter 4 / 5 / 6.0B / 6.1 / 6.2A / 6.2B contracts.
