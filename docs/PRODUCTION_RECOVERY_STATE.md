...
- **041 Production cutover second app attempt rolled back (2026-09-11):** approval `CHASUM-PO-20260911-PROD-PHASE5-041-DIRECT-CUTOVER`. 040 **never executed**. 041 **retained**. Second archive deploy of `6bf2aa0` created `dpl_7VR1peGzMg3gA2PYxXZ8Ag1AEbV8` (READY, production). Aliases reconciled 3/3; immutable-host health/build-info **PASS**; canonical-alias 403 = expected hold denial. Cron **did not** auto-align to the new host (remained on `dpl_HLeaP…`). No Cron PATCH. Approved `vercel rollback` restored `dpl_HLeaPWS86vixmStimVkfmqtvR8CH` 3/3, Cron enabled on that host, hold ON, webhooks OFF. Timezone **not** mutated. Ledger: [`docs/PHASE5_041_PRODUCTION_CUTOVER.md`](./PHASE5_041_PRODUCTION_CUTOVER.md).
- **041 Production cutover partial state (2026-09-11):** approval `CHASUM-PO-20260911-PROD-PHASE5-041-DIRECT-CUTOVER`. 040 **never executed** / superseded. 041 **applied directly and retained** (SHA256 `c2aaf4971ff72845ed9e1a77fd6f6778abc20374529d1ad88ffbf8a4ea326ec6`). PostgREST OPTIONS discovery **PASS**. First app deploy of `6bf2aa0` rolled back; serving restored to `dpl_HLeaPWS86vixmStimVkfmqtvR8CH`. Canonical-alias 403 = expected hold denial (`rule_chasum_production_recovery_hold_PqG80Y`). Hold **ON**. Cron **ENABLED**. Webhooks **OFF**. Ledger: [`docs/PHASE5_041_PRODUCTION_CUTOVER.md`](./PHASE5_041_PRODUCTION_CUTOVER.md).
...
- **Initial Production Cron restore PASSED** 2026-09-10 (PO-authorized). Enablement: `PATCH /v1/projects/prj_nUq0i5faNZTNYfQHSsLukYTW8ugm/crons` `{"enabled":true}` only; schedule unchanged `*/5 * * * *`; no `crons run`; no manual `processPendingJobs`. First scheduled execution **2026-09-10T19:55:37.499Z** GET `/api/cron/process-jobs` HTTP 200 `processed:0`. **30-minute observation COMPLETE** (19:55:37Z–20:25:37Z) via bounded historical query `--since/--until` (seven consecutive GET 200s, all `processed:0`). A later `--follow` hit Vercel’s 5-minute query duration limit and captured no rows; that limitation is **not** a Cron failure. Fresh queue recapture 2026-09-10T20:43:42Z idSha256 `480f98cf458af85045a874fdafa560fa4e505567fb6b318d7a052c4a41105bbb` unchanged vs pre-enable baseline. Held webhook `1060a548-…` attempts 0 / started_at null. Send-intent still only canary `16eeb227-…` accepted. N2 governed signals (observation window + 24h): none. Hold remained **ON** (public 403). Webhooks remained **ABSENT**. Serving remained `dpl_HLeaPWS86vixmStimVkfmqtvR8CH`. Cron **left ENABLED**. Recovery **not complete**. Hold removal **not** authorized.
...
9. Separate decision to restore Production Cron. **PASS (initial restore 2026-09-10).** Cron **ENABLED** `*/5 * * * *`. Gate 9(a) N2 **LIVE**. Gate 9(b) Package B **COMPLETE**. Gate 9(c) pre-enable + 30-minute observation **CLOSED** via bounded historical Runtime Logs (seven GET 200s, `processed:0`; fingerprint unchanged). `--follow` 5-minute limit is not a Cron failure. Webhook gate remains absent/false. Hold remains **ON**. This does **not** remove the hold and does **not** enable webhooks.
