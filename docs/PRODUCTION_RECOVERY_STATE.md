...
**Updated by:** Package B Production schema + app **COMPLETE** 2026-09-10 via Dashboard SQL Editor (`postgres` / PG 17.6) then Production deploy `dpl_HLeaPWS86vixmStimVkfmqtvR8CH` (source `57b2fce`, functional `55905a4`). Mutation-based Production synthetic **NOT RUN — NO SAFE PRE-EXISTING PRODUCTION TEST TENANT**. Cron **DISABLED**. Hold **ON**. Webhooks **ABSENT**. N2 **not live**. Recovery **not complete**.
...
- Package B communication consent **Production schema + app COMPLETE** 2026-09-10. Functional SHA `55905a44364d261a96a87fc5a1e0cbed9c168ab0` (tree `9f1e916dcdd505e9a959a3795bfcf627973b2662`). Serving `dpl_HLeaPWS86vixmStimVkfmqtvR8CH` / source SHA `57b2fce` (docs-only successor; non-doc diff empty). Dashboard SQL Editor identity: `postgres` / `postgres` / PG **17.6**. Two columns added without `IF NOT EXISTS`; `NOTIFY pgrst, 'reload schema'` after COMMIT. PostgREST **PASS** (26 rows, all `marketing_consent=false` meaning not granted/unknown historically, all `marketing_consent_at` null). Queue unchanged 581 / `480f98cf458af85045a874fdafa560fa4e505567fb6b318d7a052c4a41105bbb`. Mutation-based Production synthetic **NOT RUN — NO SAFE PRE-EXISTING PRODUCTION TEST TENANT**. Hold **ON**. Cron **DISABLED**. Webhooks **ABSENT**. No provider send. Package A and N2 remain separate. 034/035/036 remain blocked. Do not apply 027 wholesale. See [`docs/PACKAGE_B_COMMUNICATION_CONSENT.md`](./PACKAGE_B_COMMUNICATION_CONSENT.md).
...
9. Separate decision to restore Production Cron. **BLOCKED** until (a) N2 monitoring/alerting is live, (b) Package B Production schema+app is **COMPLETE** (`dpl_HLeaPWS…`; mutating synthetic remains NOT RUN), (c) no new eligible pending transactional jobs appear. Cron remains **DISABLED**.
...
Do **not** restore Cron. Package B Production two-column schema and application deploy are **COMPLETE**. Mutation-based Production synthetic remains **NOT RUN — NO SAFE PRE-EXISTING PRODUCTION TEST TENANT**; Program Lead may later decide whether a permanent governed Production test tenant should be created as separate platform/testing infrastructure. Make Claude **N2** live using existing Vercel Runtime Logs. **N4 hard gate:** before 034/035/036, `schedulingChanged` and DELETE `hasReferences` in `app/api/v1/appointments/[id]/route.ts` must be null-staff-safe. GVM remains deferred. Governance: **VERIFY CURRENT STATE BEFORE CHANGING STATE.** **NO DUPLICATE INFRASTRUCTURE OR CREDENTIAL CREATION FOR TOOLING PROBLEMS.**
