...
**Updated by:** Phase 5 / 041 Production cutover partial-state record (`CHASUM-PO-20260911-PROD-PHASE5-041-DIRECT-CUTOVER`). 041 **applied and retained**. 040 **never executed**. First app deploy of `6bf2aa0` **rolled back** to `dpl_HLeaPWS86vixmStimVkfmqtvR8CH`. Hold **ON**. Cron **ENABLED**. Webhooks **ABSENT**. App/timezone cutover still pending at second-attempt start. See [`docs/PHASE5_041_PRODUCTION_CUTOVER.md`](./PHASE5_041_PRODUCTION_CUTOVER.md). Recovery **not complete**. Hold removal is a later separate decision.
