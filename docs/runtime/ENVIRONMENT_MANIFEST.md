...
Controlled NON-PRODUCTION Preview/Staging acceptance for PR #55 is IN PROGRESS and is not hosted-acceptance PASS. Email/provider/Preview redirect gates are cleared. Current blocker is the token-hash callback amendment. Expected data plane remains isolated Staging `wnfahklzaxirftyskctd`, never Production `kxcydvhswkuzepwzzinq`. No Production/GVM mutation occurred. Do not treat a Preview hostname or successful build as Production serving identity. Never process inherited Staging queues to obtain a healthy-looking test.
