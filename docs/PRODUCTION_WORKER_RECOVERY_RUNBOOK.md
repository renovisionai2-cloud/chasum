...
| Package B communication consent | **STAGING/CODE PUBLISHED** on `cursor/package-b-communication-consent` with a bounded consent-revocation preservation follow-up. Authenticated hosted CRM action **not run**. See [`docs/PACKAGE_B_COMMUNICATION_CONSENT.md`](./PACKAGE_B_COMMUNICATION_CONSENT.md). Production schema/app rollout **not authorized**. |
