...
| Package B communication consent | **STAGING/CODE VALIDATED** on `cursor/package-b-communication-consent` (`c6c6aba`). Preview-only `dpl_5AjSrE9TBdRLBBvtQZ7m44iQc8WZ`. See [`docs/PACKAGE_B_COMMUNICATION_CONSENT.md`](./PACKAGE_B_COMMUNICATION_CONSENT.md). Production schema/app rollout **not authorized**. |
