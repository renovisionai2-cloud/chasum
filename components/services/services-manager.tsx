  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <ColorPicker name="color" colors={SERVICE_COLORS} defaultValue={service?.color ?? SERVICE_COLORS[0]} />
      </div>
      {service && (
      ...
      <FormFooter onCancel={onClose} pending={pending} submitLabel={service ? "Update" : "Create"} />
    </form>
  );
}
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-3">
                    <span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: service.color }} aria-hidden="true" />
                  ...
                  <div className="flex shrink-0 gap-1">
                    <IconButton label={`Edit ${service.name}`} onClick={() => { setEditing(service); setOpen(true); }}>
                      <Pencil className="h-4 w-4" />
                    </IconButton>
                    <IconButton label={`Delete ${service.name}`} className="text-destructive hover:text-destructive" onClick={() => handleDelete(service.id, service.name)}>
