import { planDisplayName } from "@/lib/billing/plan-entitlements";
import {
  addBillingPeriod,
  comparePlans,
  FALLBACK_PLANS,
...
function mapPlan(row: Record<string, unknown>): BillingPlan {
  const planKey = asPlanKey(row.plan_key);
  return {
    planKey,
    name: planDisplayName(planKey),
...
export async function getBillingSummary(
  ...
  const { data: business, error } = await supabase
    .from("businesses")
    .select(
      "id, name, subscription_plan_key, subscription_status, billing_interval, trial_starts_at, trial_ends_at, current_period_start, current_period_end, cancel_at_period_end, canceled_at, stripe_customer_id, stripe_subscription_id, private_alpha_enabled",
    )
    .eq("id", businessId)
    .single();
  ...
  return {
    ...
    privateAlphaEnabled: Boolean(
      (business as { private_alpha_enabled?: boolean }).private_alpha_enabled,
    ),
  };
}
...
export async function downloadInvoicePayload(invoice: BillingInvoice): Promise<{
  ...
  const lines = [
    ...
    "",
    "Generated as a local billing record.",
    "Provider-hosted invoices appear only when live subscription billing is connected.",
  ];
