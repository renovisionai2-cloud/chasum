...
/** When a brief UI acknowledgement already played, drop the engine understand beat. */
const STRIP_UNDERSTAND: RegExp[] = [
  /^Got it — [^.]+.\s*/i,
  /^Noted — [^.]+.\s*/i,
  /^Understood — [^.]+.\s*/i,
  /^Thanks, [^.]+.\s*/i,
  /^I've taken that in — [^.]+.\s*/i,
  /^I've learned you're working with [^.]+.\s*/i,
  /^I see you're currently on [^.]+.\s*/i,
  /^A team of about [^.]+ — that shapes how I'll advise you.\s*/i,
  /^Thank you, [^.]+.\s*/i,
];
 ...
 */
export function presentConsultationReply(
  content: string,
  options?: { stripLeadingUnderstand?: boolean },
): string {
  let text = content.trim();
  if (!text) return content;

  if (options?.stripLeadingUnderstand) {
    for (const re of STRIP_UNDERSTAND) {
