...
export async function sendInvoiceEmail(input: {
  ...
  const context: AppointmentTemplateContext = {
    ...
    depositPaidCents: Math.max(0, inv.amountPaidCents - inv.amountRefundedCents),
    remainingBalanceCents: model.collectibleRemainingCents,
    invoiceNumber: inv.invoiceNumber,
    paymentStatusLabel: model.statusLabel,
    notes: model.notes,
    invoicePaidCents: Math.max(0, inv.amountPaidCents - inv.amountRefundedCents),
    invoiceBalanceCents: model.collectibleRemainingCents,
