  ...
  createServiceClient: () => ({
    from(table: string) {
      ...
      query.maybeSingle = async () => {
        if (table === "appointments") {
          const result = await (query.single as () => Promise<{data: object}>)();
          return { ...result, data: { ...result.data, location_id: "loc-1", customer_id: "cust-1", service_id: "svc-1" } };
        }
        const rows: Record<string, object> = {
          customers: { id: "cust-1", name: "Test Customer", email: "guest@example.com", phone: null },
          services: { id: "svc-1", name: "Ultrasound" },
          staff: { id: "staff-1", name: "Alex Rivera", email: null },
