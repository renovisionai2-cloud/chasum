...
describe("public booking 12:10 time-integrity diagnostic", () => {
  ...
  it("Review formats the selected instant in the operational timezone", () => {
    expect(PAGE_SOURCE).toContain(
      'formatBusinessDate(selectedSlot.start, { timezone }',
    );
    expect(PAGE_SOURCE).toContain("{formatBusinessTime(selectedSlot.start, { timezone })}");
